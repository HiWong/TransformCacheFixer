...
public class IntermediateFolderUtils {
    ...
    public synchronized File getContentLocation(
        ...
        /////////////////////start of allen debug code///////
        if("androidx.legacy:legacy-support-v4:1.0.0".equals(name)){
            System.out.println("Attention!now ");
        }
        ///////////////////////end of allen debug code///////
    ...
    public void record(File recordDir, String transformName) {

        File outRecordFile = new File(recordDir, transformName + "_output.txt");
        if (outRecordFile.exists()) {
            outRecordFile.delete();
        }

        try {
            outRecordFile.createNewFile();

            BufferedWriter writer = new BufferedWriter(new FileWriter(outRecordFile));
            PrintWriter printWriter = new PrintWriter(writer);

            List<SubStream> dirStreams = new ArrayList<>();

            printWriter.println("===========start of jar format substreams=======");

            for (SubStream stream : subStreams) {
                if (stream.getFormat() == Format.DIRECTORY) {
                    dirStreams.add(stream);
                } else {
                    printWriter.println(stream.toString());
                }
            }
            printWriter.println("============start of dir format substreams==========");
            for (SubStream dirStream : dirStreams) {
                printWriter.println(dirStream.toString());
            }
