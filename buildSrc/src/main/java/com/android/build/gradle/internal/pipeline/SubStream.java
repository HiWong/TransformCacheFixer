...
public final class SubStream {
    ...
    /**
     * 只针对CLASSES类型，否则stripDebugSymbol transform会出错
     *
     * @return
     */
    private String computeDirName() {
        if (types.contains(QualifiedContent.DefaultContentType.CLASSES)) {
            return index + "_" + System.currentTimeMillis();
        } else {
            return Integer.toString(index);
        }
    }
    ...
    public SubStream duplicateWithPresent(boolean exists) {
        ...
        return new SubStream(name, filename, index, scopes, types, format, exists);
    }
