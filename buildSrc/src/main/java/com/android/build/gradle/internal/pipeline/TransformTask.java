...
public class TransformTask extends StreamBasedTask implements Context {
        ...
        recorder.record(
                ...
                new Recorder.Block<Void>() {
                    ...
                    public Void call() throws Exception {
                        ...
                        /////////////////start of record code//////////////////
                        outputConsumedInputs(consumedInputs.getValue());
                        /////////////////end of record code///////////////////
    ...
    //////////////////////////////////////////start of transform record code////////////////////////
    private File getTransformRecordDir() {
        File recordDir = new File(getProject().getBuildDir().getPath() + File.separator + "transformRecord" + File.separator);
        if (!recordDir.exists()) {
            recordDir.mkdirs();
        }
        return recordDir;
    ...
    private boolean isClassConsumedTransform() {
        return transform.getInputTypes().contains(QualifiedContent.DefaultContentType.CLASSES);
    }

    private void outputConsumedInputs(List<TransformInput> inputs) {

        if (!isClassConsumedTransform()) {
            return;
        }

        File transformRecordDir = getTransformRecordDir();

        File currentTransformRecordFile = new File(transformRecordDir, transform.getName() + "_input.txt");
        if (currentTransformRecordFile.exists()) {
            currentTransformRecordFile.delete();
        }
        try {
            currentTransformRecordFile.createNewFile();

            BufferedWriter writer = new BufferedWriter(new FileWriter(currentTransformRecordFile));
            PrintWriter printWriter = new PrintWriter(writer);

            //TODO 怎么跟index对应起来呢？
            for (TransformInput input : inputs) {
                printWriter.println("=============start of jarInputs=============");
                for (JarInput jarInput : input.getJarInputs()) {
                    printWriter.println(jarInput.toString());
                }

                printWriter.println("==============start of dirInputs============");
                for (DirectoryInput dirInput : input.getDirectoryInputs()) {
                    printWriter.println(dirInput.toString());
                }
            }
    ...
    private void outputOutStream() {
        if (!isClassConsumedTransform()) {
            return;
        }
        if (outputStream != null) {
            outputStream.record(getTransformRecordDir(), transform.getName());
        }
    }
